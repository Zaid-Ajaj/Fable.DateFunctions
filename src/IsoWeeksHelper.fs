...
[<AutoOpen>]
module IsoWeeksHelperDateTimeOffset =
    type DateTimeOffset with
        /// Get the number of calendar ISO weeks between the given dates.
        member inline date.DifferenceInCalendarISOWeeks(otherDate: DateTimeOffset) : int = ExternalDateFns.differenceInCalendarISOWeeks date otherDate
        /// Return the end of an ISO week for the given date. The result will be in the local timezone.
        member inline date.EndOfIsoWeek() : DateTimeOffset = DateTimeOffset(ExternalDateFns.endOfIsoWeek date)
        member inline date.IsInTheSameISOWeekAs(otherDate: DateTimeOffset) : bool = ExternalDateFns.isSameIsoWeek date otherDate
        member inline date.IsInThisISOWeek() : bool = ExternalDateFns.isThisIsoWeek date
        member inline date.GetISOWeek() : int = ExternalDateFns.getISOWeek date
        member inline date.LastDayOfISOWeek() : DateTimeOffset = DateTimeOffset(ExternalDateFns.lastDayOfIsoWeek date)
        member inline date.StartOfISOWeek() : DateTimeOffset = DateTimeOffset(ExternalDateFns.startOfIsoWeek date)
        /// Set the ISO week to the given date, saving the weekday number.
        member inline date.SetISOWeek(n : int) : DateTimeOffset = DateTimeOffset(ExternalDateFns.setIsoWeek date n)